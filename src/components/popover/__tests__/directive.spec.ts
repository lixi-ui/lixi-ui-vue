...
const Comp = {
  template: `
  <lx-popover title="title" :content="content" ref="popover"/>
...
describe('v-popover', () => {
  ...
  test('should render correctly', () => {
    ...
    expect(document.body.querySelector('.lx-popover').innerHTML).toContain(AXIOM)
    wrapper.unmount()
  })
  ...
  test('should show popover when reference is mounted', async () => {
    ...
    expect(document.body.querySelector('.lx-popover').getAttribute('style')).toContain('display: none')
    await wrapper.find(refNode).trigger('click')
    await rAF()
    await nextTick()
    expect(document.body.querySelector('.lx-popover').getAttribute('style')).not.toContain('display: none')
    wrapper.unmount()
  })
